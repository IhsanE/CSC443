...
int main(int argc, const char * argv[]) {
    ...
    while (std::getline(csv_file, record)) {
        ...
        if (page_full == -1) {
            if (free_slot == -1) {
                page_file.write((const char *) page->data ,fixed_len_sizeof(&r)*page->num_records);
            } else {
                page_file.write((const char *) page->data ,fixed_len_sizeof(&r)*(free_slot - 1));
            }
            free(page->data);
            init_fixed_len_page(page,  page_size, fixed_len_sizeof(&r));
            num_pages++;
    ...
    if (fixed_len_page_freeslots(page) >= 0) {
        int free_slot = find_first_free_slot(page);
        if (free_slot == -1) {
            page_file.write((const char *) page->data ,(NUM_ATTR*ATTR_SIZE)*page->num_records);
        } else {
            page_file.write((const char *) page->data ,(NUM_ATTR*ATTR_SIZE)*(free_slot - 1));
        }
    }
